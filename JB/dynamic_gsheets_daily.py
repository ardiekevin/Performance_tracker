...
if os.path.isfile(file_path):
    ...
    df9 = df9.sort_values(by='Date',ascending=False)
...
missing_dates_df9 = unique_dates_df1 - unique_dates_df9
...
for missing_dates_df9 in missing_dates_df9:
    row_data = {
        'count':0, 
        'RAF Commission':0.00
    }
    df9 = df9._append(row_data, ignore_index=True)
#------------------------------------------------------------------------------------------------------------------------------5 start
...
appended_rows = pd.DataFrame(columns=['Date', 'game_type_name','Number of Unique Player', 'Total Turnover', 'Profit/Loss', 'Turnover Margin (%)'])
