...
def run_script():
    ...
    selected_frequency = frequency_var.get() #frequency_combobox.get()
    ...
    if selected_currency == "JEETBUZZ BDT":
        spreadsheet_name = "JeetBuzz 2024 Biz Performance Tracker - BDT"
    elif selected_currency == "JEETBUZZ INR":
        spreadsheet_name = "JeetBuzz 2024 Biz Performance Tracker - INR"
    elif selected_currency == "JEETBUZZ PKR":
        spreadsheet_name = "JeetBuzz 2024 Biz Performance Tracker - PKR"
    else:
        return

    confirmation = messagebox.askyesno("Confirmation", "Are you sure you want to run the script?")
    if confirmation:
        subprocess.Popen(["python", rf"C:\Users\ardie.asilo\Desktop\Performance_Tracker\JB\{script_name}", "--spreadsheet", spreadsheet_name])
...
currency_combobox = ttk.Combobox(root, values=["JEETBUZZ BDT", "JEETBUZZ INR", "JEETBUZZ PKR"], width=30, height=20, style = "TCombobox")
