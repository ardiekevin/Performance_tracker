...
client = pygsheets.authorize(service_file=r'C:\Users\ardie.asilo\Desktop\Performance_Tracker\S6\my-test-project-416604-de901d750987.json')
...
missing_types_df5 = set(product_type_order) - set(df5['game_type_name'])
appended_rows = pd.DataFrame(columns=['Date', 'game_type_name','Number of Unique Player', 'Total Turnover', 'Profit/Loss', 'Margin'])
for date in unique_dates_df1:
    if date not in unique_dates_df5:
        for product_type in product_type_order:
            new_row = {'Date': date, 'game_type_name': product_type}
            new_row.update(row_data)
            appended_rows = appended_rows._append(new_row, ignore_index=True)
    else:
        for product_type in product_type_order:
            if product_type in missing_types_df5 or not ((df5['Date'] == date) & (df5['game_type_name'] == product_type)).any():
                new_row = {'Date': date, 'game_type_name': product_type}
                new_row.update(row_data)
                appended_rows = appended_rows._append(new_row, ignore_index=True)
output_df5 = pd.concat([df5, appended_rows])
output_df5['game_type_name'] = pd.Categorical(output_df5['game_type_name'], categories=product_type_order, ordered=True)
output_df5 = output_df5.sort_values(by=['game_type_name', 'Date'], ascending=[True, False])
selected_columns = [
...
selected_df5 = output_df5.groupby('game_type_name').apply(lambda x: x[selected_columns].T).reset_index(level=1, drop=True).reset_index()
selected_df5.drop(columns=['game_type_name'], inplace=True)
...
selected_columns = [
    'Total Bonus Cost', 
    'Total Bonus Claimed', 'Total Unique Player Claimed'
]
