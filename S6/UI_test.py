...
def run_script():
    ...
    if selected_currency == "SIX6S BDT":
        spreadsheet_name = "6s 2024 Biz Performance Tracker - BDT"
    elif selected_currency == "SIX6S INR":
        spreadsheet_name = "6s 2024 Biz Performance Tracker - INR"
    elif selected_currency == "SIX6S PKR":
        spreadsheet_name = "6s 2024 Biz Performance Tracker - PKR"
    else:
        return

    confirmation = messagebox.askyesno("Confirmation", "Are you sure you want to run the script?")
    if confirmation:
        subprocess.Popen(["python", rf"C:\Users\ardie.asilo\Desktop\Performance_Tracker\S6\{script_name}", "--spreadsheet", spreadsheet_name])
...
currency_combobox = ttk.Combobox(root, values=["SIX6S BDT", "SIX6S INR", "SIX6S PKR"], width=30, height=20, style = "TCombobox")
